...
Purpose: Mark a task as incomplete by ID or title
"""
...
async def incomplete_task(
    ...
    task_id: Optional[int] = Field(
        description="Task ID to mark as incomplete", default=None
    ),
    ...
    """
    Mark a task as incomplete by ID or title.

    Args:
        user_id: User's ID from JWT context
        task_id: Task ID to mark as incomplete (optional if task_title provided)
    ...
    try:
        ...
        if task_id is None and not task_title:
            return {
                "error": "Please provide either a task ID or task title to mark as incomplete",
                "status": "error",
            }
        ...
        with get_session_context() as db:
            ...
            task_toggle = TaskToggleComplete(completed=False)
            ...
            # If exactly one match, mark it incomplete
